...
class TestComputeWageIds(TestSwissdecCommon):
    ...
    def setUpClass(cls):
        ...
        cls.hourly_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_hours",
            raise_if_not_found=False,
        )
        cls.overtime_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_hours_100",
            raise_if_not_found=False,
        )
        cls.overtime_125_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_hours_125",
            raise_if_not_found=False,
        )
        cls.overtime_150_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_hours_150",
            raise_if_not_found=False,
        )
        cls.overtime_200_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_hours_200",
            raise_if_not_found=False,
        )
        cls.on_call_duty_125_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_on_call_125",
            raise_if_not_found=False,
        )
        cls.night_shift_110_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_night_110",
            raise_if_not_found=False,
        )
        cls.lesson_input_type = cls.env.ref(
            "l10n_ch_hr_payroll.l10n_ch_elm_input_WT_lessons",
            raise_if_not_found=False,
        )
        ...
        cls.employee_monica.create_version(
            {
                ...
                "contract_date_end": date(2023, 12, 31),
                "wage": 5000,
                "hourly_wage": 100,
                "wage_type": "monthly",
    ...
    def test_compute_wage_ids_1(self):
        ...
        payslip_jan = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 1, 1), date(2023, 1, 31), company.id
        )
        payslip_jan_results = [
            ("CH_UNPAID", 806.45),
        ...
        payslip_feb = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 2, 1), date(2023, 2, 28), company.id
        )
        payslip_feb_results = [
            ("CH_1000", 2857.14),
            ("CH_ILLNESS_HOURLY", 0.0),
        ...
        payslip_mar = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 3, 1), date(2023, 3, 31), company.id
        )
        payslip_mar_results = [
        ...
        payslip_apr = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 4, 1), date(2023, 4, 30), company.id
        )
        payslip_apr_results = [
            ('CH_Interruption', 0.0),
            ('CH_1000', 0.0),
        ...
        payslip_may = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 5, 1), date(2023, 5, 31), company.id
        )
        payslip_may_results = [
    ...
    def test_compute_wage_ids_3(self):
        ...
        payslip_jun = self._l10n_ch_generate_swissdec_demo_payslip(
            version, date(2023, 6, 1), date(2023, 6, 30), company.id
        )
        payslip_jun_results = [
            ("CH_1005", 0),
            ("CH_1068", 2000.0),
