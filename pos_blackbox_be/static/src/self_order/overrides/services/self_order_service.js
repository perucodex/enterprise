...
patch(SelfOrder.prototype, {
    async setup(...args) {
        ...
        if (this.config.iface_fiscal_data_module) {
    ...
    initData() {
        ...
        if (
            ...
            const fiscal_data_category_ids = this.config.work_in_product.pos_categ_ids;
            this.productCategories = this.productCategories.filter(
                (category) => !fiscal_data_category_ids.includes(category.id)
            );
        }
    },
});
