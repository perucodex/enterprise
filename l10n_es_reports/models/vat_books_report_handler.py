...
class L10n_EsVATBooksReportHandler(models.AbstractModel):
    ...
    def _query_invoices(self, report, options, offset=0, load_more_limit=0):
        ...
        for journal_type in invoice_data:
            self.env.cr.execute(SQL("""
                SELECT
                    ...
                    SUM(CASE WHEN account_move_line.tax_line_id IS NOT NULL
                        AND (tax.l10n_es_type IS NULL OR tax.l10n_es_type != 'retencion')
    ...
    def _report_expand_unfoldable_line_vat_books_section(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data=None):
        ...
        for result_journal_type, invoice_data_list in self._query_invoices(report, options, offset, report.load_more_limit):
            if result_journal_type == section_type:
                ...
                for invoice_data in invoice_data_list:
                    if options['export_mode'] is None and report.load_more_limit and results_count == report.load_more_limit:
                        has_more = True
                        break

                    new_line = self._get_report_line_invoice(options, invoice_data, line_dict_id)
                    lines.append(new_line)
                    results_count += 1

                return {
                    'lines': lines,
                    'offset_increment': results_count,
                    'has_more': has_more,
                }
    ...
    def _l10n_es_libros_get_tax_period_vals(self, company, date):
        if company.account_return_periodicity == 'monthly':
            period = f'{date.month:02d}'
        else:
            period = f'{get_quarter_number(date)}T'
        return {
            'year': date.year,
            'period': period,
        }
    ...
    def _l10n_es_fill_common_line_vals(self, company, partner, date, delivery_date, invoice_type, operation_code, amount):
        ...
        common_line_vals.update(self._l10n_es_libros_get_tax_period_vals(company, date))
    ...
    def _l10n_es_libros_fill_header(self, sheet_income, sheet_expense):
        def get_headers(is_income):
            headers = [
                ('Autoliquidación', ('Ejercicio', 'Periodo')),
                ('Actividad', ('Código', 'Tipo', 'Grupo o Epígrafe del IAE')),
                ('Tipo de Factura', None),
                ('Concepto de Ingreso' if is_income else 'Concepto de Gasto', None),
                ('Ingreso Computable' if is_income else 'Gasto Deducible', None),
                ('Fecha Expedición', None),
                ('Fecha Operación', None),
            ]
            if is_income:
                headers.extend([
                    ('Identificación de la Factura', ('Serie', 'Número', 'Número-Final')),
                    ('NIF Destinario', ('Tipo', 'Código País', 'Identificación')),
                    ('Nombre Destinario', None),
                ])
            else:
                headers.extend([
                    ('Identificación Factura del Expedidor', ('(Serie-Número)', 'Número-Final')),
                    ('Fecha Recepción', None),
                    ('Número Recepción', None),
                    ('Número Recepción Final', None),
                    ('NIF Expedidor', ('Tipo', 'Código País', 'Identificación')),
                    ('Nombre Expedidor', None),
                ])
            headers.append(('Clave de Operación', None))
            if is_income:
                headers.extend([
                    ('Calificación de la Operación', None),
                    ('Operación Exenta', None),
                ])
            else:
                headers.extend([
                    ('Bien de Inversión', None),
                    ('Inversión del Sujeto Pasivo', None),
                    ('Deducible en Periodo Posterior', None),
                    ('Periodo Deducción', ('Ejercicio', 'Periodo')),
                ])
            headers.extend([
                ('Total Factura', None),
                ('Base Imponible', None),
            ...
            if is_income:
                headers.append(('Cuota IVA Repercutida', None))
            else:
                headers.extend([
                    ('Cuota IVA Soportado', None),
                    ('Cuota Deducible', None),
                ])
            headers.extend([
                ('Tipo de Recargo eq.', None),
                ('Cuota Recargo eq.', None),
            ])
            if is_income:
            ...
            headers.extend([
                (head, ('Fecha', 'Importe', 'Medio Utilizado', 'Identificación Medio Utilizado')),
                ('Tipo Retención del IRPF', None),
                ('Importe Retenido del IRPF', None),
                ('Registro Acuerdo Facturación', None),
                ('Inmueble', ('Situación', 'Referencia Catastral')),
                ('Referencia Externa', None),
            ])
            return headers

        for sheet, is_income in [(sheet_income, True), (sheet_expense, False)]:
            headers = get_headers(is_income)
            col_idx = 0
            for header_title, subheaders in headers:
                if not subheaders:
                    sheet.merge_range(0, col_idx, 1, col_idx, header_title)
                    col_idx += 1
                else:
                    sheet.merge_range(0, col_idx, 0, col_idx + len(subheaders) - 1, header_title)
                    col_idx += len(subheaders)
            col_idx = 0
            for header_title, subheaders in headers:
                if not subheaders:
                    col_idx += 1
                    ...
                    for subheader in subheaders:
                        sheet.write(1, col_idx, subheader)
                        col_idx += 1
    ...
    def _l10n_es_libros_merge_line_tax(self, line_vals, line, tax, tax_amount):
        if tax.l10n_es_type == 'recargo':
            ...
            if not self._l10n_es_libros_is_income_line(line) and any('mod303' in tag for tag in line.tax_tag_ids.mapped('name')):
    ...
    def _l10n_es_libros_get_sheet_line_vals(self, lines):
        ...
        base_lines = lines.filtered_domain([('tax_ids', '!=', False)])
        ...
        # of the base amount
        tax_lines = lines.filtered_domain([('tax_line_id', '!=', False)])
    ...
    def _l10n_es_libros_fill_content(self, sheet_income, sheet_expense, report, options):
        ...
        def _process_and_write_batch(current_batch_lines):
            batch_inc, batch_exp = self._l10n_es_libros_get_sheet_line_vals(current_batch_lines)
            sheet_configs = (
                {'sheet': sheet_income, 'line_vals': batch_inc, 'type': 'inc', 'fields': INCOME_FIELDS},
                {'sheet': sheet_expense, 'line_vals': batch_exp, 'type': 'exp', 'fields': EXPENSE_FIELDS}
            )
        ...
        while move_groups := self.env['account.move.line']._read_group(
            domain,
            groupby=['move_id'],
            ...
            current_batch_lines = self.env['account.move.line'].concat(*(lines for _move, lines in move_groups))
            _process_and_write_batch(current_batch_lines)
            if len(move_groups) < batch_size:
                break
    ...
    def export_libros_de_iva(self, options):
        ...
        workbook = xlsxwriter.Workbook(output, {'constant_memory': True, 'strings_to_formulas': False})
